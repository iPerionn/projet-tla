...
import analyses.Interpretation;
...
public class Plateau {
    ...
    public final static int LARGEUR_PLATEAU = 20; // largeur du plateau en nombre de carreau
    final static int HAUTEUR_PLATEAU = 14; // hauteur du plateau en nombre de carreau

    // grille

    public Carreau carreaux[] = new Carreau[LARGEUR_PLATEAU * HAUTEUR_PLATEAU];
    ...
    void setNiveau(Interpretation interpretation) {
    ...
    void start() {
        ...
        char[] ch = interpretation.getWalls();
        ...
        for(Commutateur commutateur: interpretation.getCommutateurs()) {
            commutateur.reset();
        ...
        for(Fantome fantome: interpretation.getFantomes()) {
            fantome.reset();
        ...
        joueur_x = interpretation.getxSpawn();
        joueur_y = interpretation.getySpawn();
        ...
        interpretation.hookApresDeplacement(this);
    ...
    private void deplacement(Direction direction, int destination_x, int destination_y) {
        ...
        Trappe trappe = interpretation.getTrappes().stream()
                .filter(t ->
    ...
    private void apresDeplacement() {
        ...
        if (detecteCollisionFantome()) {
            ...
            Commutateur commutateur = interpretation.getCommutateurs().stream()
                    .filter(c ->
